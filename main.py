...
# --- Telegram Bot Token ---
TELEGRAM_TOKEN = "8217530229:AAHekGlqu9BdX087MFK5RFXtPASX-MsPckk"  # Replace with your token

# ----------------- Dummy background task -----------------
async def background_task():
    """A fake async task that prints a heartbeat every 10 seconds."""
    while True:
        print("⏱  Background task alive")
        await asyncio.sleep(10)

# ----------------- Telegram commands -----------------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 Example bot running on Railway!\nCommands:\n"
        "/status – show if alive\n/run – start background task"
    )

async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🟢 Bot deployed and listening.")

async def run_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🚀 Starting background task…")
    asyncio.create_task(background_task())

# ----------------- Dummy HTTP server (keeps Railway alive) -----------------
def keep_alive():
...
async def main_tg():
    ...
    print("✅ Telegram bot ready. Type /start in Telegram.")
