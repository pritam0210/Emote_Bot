...
async def GetLoginData(base_url, payload, token):
    url = f"https://client.ind.freefiremobile.com/GetLoginData"
    ssl_context = ssl.create_default_context()
    ssl_context.check_hostname = False
    ssl_context.verify_mode = ssl.CERT_NONE
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip().startswith('@a'):
                            ...
                            try:
                                ...
                                message = '[B][C]{}\nACITVE TarGeT -> {}\n'.format(get_random_color(), xMsGFixinG(uid))
...
async def MaiiiinE():
    Uid , Pw = '4240025658' , 'E7BAFEC9A103CF61803309E8A6B6CF03A54BDBE8C04CE1880E1C457D8CCDEE50'
    ...
    os.system('clear')
    print(render('BooT', colors=['white', 'red'], align='center'))
    print('')
    print(f" - BoT STarTinG And OnLine on TarGet : {TarGeT}\n")
...
TELEGRAM_TOKEN = "8217530229:AAHekGlqu9BdX087MFK5RFXtPASX-MsPckk"
OWNER_CHAT_ID = None  # Optional: put your Telegram numeric ID here if you want private alerts

# Helper: Send Telegram alert messages (optional)
def send_tg_message(msg):
    try:
        if OWNER_CHAT_ID:
            requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage?chat_id={OWNER_CHAT_ID}&text={msg}")
    except Exception as e:
        print("Telegram message send failed:", e)

# /start command handler
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🤖 FF Bot Control Active!\nUse /run to start the Free Fire bot.")

# /status command handler
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🟢 Bot deployed and awaiting commands.")

# /run command handler - starts your existing Free Fire bot
async def run_ffbot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🚀 Launching Free Fire bot in background...")
    asyncio.create_task(StarTinG())  # runs your FF bot async loop without blocking Telegram
...
async def main_tg():
    ...
    app.add_handler(CommandHandler("run", run_ffbot))
    print("✅ Telegram bot is running. Open Telegram and type /start to begin.")
